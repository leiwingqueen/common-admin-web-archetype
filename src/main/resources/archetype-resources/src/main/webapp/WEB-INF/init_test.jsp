...
<script>
	...
	function searchData(page, disR) {
		...
		var req = 'page=' + (page || 1) + '&displayRecord='
				+ (disR = disR || 100);
		${symbol_dollar}('.search_box').find('input,select').each(function() { // 遍历搜索框输入的内容作为参数
			if (this.value && this.value != '')
				req += '&' + this.id + '=' + this.value;
		});
		var url = '/workflow/claimTaskList.do';
		${symbol_dollar}.getJSON(url,req,function(json) {
			if (!json.success) {
				alert(json.message);
				return;
			}
			...
			${symbol_dollar}.tools.handleHTML(${symbol_dollar}('${symbol_pound}taskListTbody'), html);
			// 分页
			${symbol_dollar}.tools.commonPage('${symbol_pound}common_page', fn, disR, count,p, pCount);
		})
	}
	//查找我的任务
	function searchMyTaskData(page, disR) {
		...
		${symbol_dollar}.getJSON(url,req,function(json) {
			...
			${symbol_dollar}.tools.handleHTML(${symbol_dollar}('${symbol_pound}myTaskTbody'), html);
			// 分页
			${symbol_dollar}.tools.commonPage('${symbol_pound}common_page', fn, disR, count,p, pCount);
		});
	}
	...
	function searchMyApplyData(page, disR) {
			...
			${symbol_dollar}.ajax({
				...
				success:function(json){
					...
					${symbol_dollar}.tools.handleHTML(${symbol_dollar}('${symbol_pound}myApplyTbody'), html);
					// 分页
					${symbol_dollar}.tools.commonPage('${symbol_pound}common_page', fn, disR, count,p, pCount);
				}
			});
	...
	function claim(taskId){
		...
		//alert(taskId);
		${symbol_dollar}.getJSON('/workflow/claim.do','taskId='+taskId,function(json){
			if(!json.success){
				alert(json.message);
				return;
			}
		});
		${symbol_dollar}.dialog({
			title:'提示',
		    content:'签收成功',
		    time: 1000
		});
