...
var toolsApp = {
	...
	add:function(data){
		...
		$('#drag_area').not('.no_drag').sortable();
	},
...
$(function(){
	...
	//显示日历 年月日 2015-01-13
	$('.J_calendar_date').on('click',function(){
		$.calendar({ format:'yyyy-MM-dd' });
	})
	$('.J_calendar_date').css('cursor','default')
	//左边树菜单
//	alert("ok"+sessionStorage.menuData);
...
$.multiSelect = {
		...
		init: function(id, value) {
			if (this.list && this.list.length > 0) {
				this.initMultiSelect(id, value);
				return;
			}
			var thiz = this;
			$.getJSON('/loanApply/listUser.do?userType=0', function(data) {
				if(thiz.list && thiz.list.length > 0) { // 最后去重检测：由于异步特性，在加载完毕后，list数据有可能被外部更改
					thiz.initMultiSelect(id, value);
					return;
				}
				var user = data.object;
				for(var i in user) {
					var usr = user[i];
					//if (usr.user_name && usr.user_name.length > 3 && usr.user_name.indexOf('dw_') == 0) {
						thiz.list.push(usr);
					//}
				}
				//thiz.list = data.object;
				thiz.initMultiSelect(id, value);
			});
		},
		initMultiSelect: function(id, value) {
			var html = '<option value=0>请选择</option>';
			$.each(this.list, function(i, item) {
				if (!item.realName || item.realName == '') return true;
				var val = value && value.length > 0 ? value.split(',') : [];
				var s = '';
				for(var j in val) {
					if (item.userId == val[j]) {
						s = ' selected';
						break;
					}
				}
				html += '<option value="' + item.userId + '"' + s + '>' + item.realName + '</option>';
			});
			$('#' + id).html(html);
			$('#' + id + '.chzn-select').trigger("liszt:updated"); // 2012-11-08
			$('#' + id + '.chzn-select').chosen();
		}
	};
